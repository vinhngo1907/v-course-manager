...
export class LessonService {
    ...
    async updateStatus({ lessonId, accountId, published }: LessonUpdateStatusDto) {
        if (!accountId) throw new ForbiddenException("You don't have permission to update not own lesson")
        ...
        if (!existedCourse) throw new CourseNotFoundException("Course does not exist with user " + accountId);
        
        const existedLesson = await this.databaseService.lesson.findFirst({
            where: { id: lessonId, courseId: existedCourse.id}
        });

        if(!existedLesson) throw new NotFoundException(`This lesson ${lessonId} not found`);
